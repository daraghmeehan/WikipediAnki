...
all_easy_csvs = [easy_csv for easy_csv in glob.glob("(Easy)*.csv")]
...
if all_easy_csvs != []:
    ...
    num_easy_cards = sum(
        [
            int(re.match("\(Easy\).* (\d+) card(s)?\.csv", easy_csv).groups()[0])
...
all_normal_csvs = [normal_csv for normal_csv in glob.glob("(Normal)*.csv")]

if all_normal_csvs != []:

    num_normal_cards = sum(
        [
            int(re.match("\(Normal\).* (\d+) card(s)?\.csv", normal_csv).groups()[0])
