...
import time  # for timestamping csvs of flashcards
from pathlib import Path  # for writing flashcards to "Flashcards" folder

from sparql_ui import query_wikidata_with_sparql # importing our SPARQL query tool UI

nlp = spacy.load("en_core_web_sm")  # loading the language model to split sentences and find named entities
...
def run_wikipedianki() -> None:
    ...
    # our flashcard dataframe which we will later output to a csv
    ...
    try:
        while True:
            ...
            if next_action == "s":
                ...
                # create all cards for given page
                new_cards = create_new_cards(wikipedia_url, soup)

                # add all new cards to our dataframe
            ...
            # try a sparql query
                ...
                question, answer, page_title, wikidata_url = query_wikidata_with_sparql()
                if question == "":
                    # if our question is empty, we haven't chosen a question/answer pair to make a flashcard from
                    continue
                else:
                    # allow our user to customise the flashcard returned from the NL -> SPARQL query tool
                    hint = ""
                    extra_info = ""
                    ...
                    card["URL"] = wikidata_url

                    card = pd.DataFrame(card)

                    # add our new flashcard to our collection
                    flashcards = pd.concat([flashcards, card], ignore_index=True)

            # exit and save all flashcards
            elif next_action == "b":
                # exit condition
                break
    ...
    # save our flashcard to a csv to then import in Anki
...
def choose_next_action():
    ...
    next_action = input(
        "What would you like to do?\nType 's' to study a page, 'e' to edit created flashcards, 'q' to query Wikidata, 'b' to exit and save.\n"
    )

    if next_action in ["s", "e", "b", "q"]:
        return next_action
    else:
...
def choose_page() -> tuple[str, BeautifulSoup]:
    ...
    try:
        r = requests.get(wikipedia_url, allow_redirects=True) # download our Wikipedia page
        soup = BeautifulSoup(r.content, "html.parser", from_encoding="utf-8") # parse the html with BeautifulSoup
...
def study_page(soup: BeautifulSoup) -> list[dict[str, any]]:
    ...
    doc = nlp(text)  # doing sentence splitting, tokenisation, and named entity recognition
...
def create_next_card(
    ...
    # the user chooses which of the lines presented to make a card from
    line_number_to_learn = choose_line(matching_sents)
    if line_number_to_learn == -1:
        return create_next_card(sents, page_title)

    # selecting the chosen line
...
def choose_line(sents: list[spacy.tokens.span.Span]) -> int:
    ...
    # printing each of the lines with their corresponding line number
...
def create_card_from_line(
    ...
    # the user selects which words to hide in the sentence
    ...
    # adding found named entities into extra info field
    named_entities = [entity.text for entity in nlp(line_to_learn.text).ents]
    unique_named_entities = set(named_entities)
    if len(unique_named_entities) == 0:
        extra_info == ""
    else:
        extra_info = "Entities found:\n" + "\n".join(unique_named_entities)
    ...
    # allows the user to tag a card as easy, which they can add to a separate deck of cards with much longer time intervals
    card["is_easy"] = confirm_with_user("\nFinally, is this an easy card?")
...
def extract_answers_from_line_to_learn(
    ...
    all_answer_positions = (
        []
    )  # to track all the token indices in the sentence to be removed (i.e. every position in the sentence that will be hidden)
    span_numbers = (
        {}
    )  # to track, for a given index (position in a sentence), which span group (i.e. the answer) it corresponds to
    answer_strings = []  # to collect the answers for the given subspans

    # reminder - selected_spans is a list of spans, e.g. [[1,2], [5,6]], or simply [[1,2,3]]
    all_answer_positions = [
...
def build_question(
    ...
    # cleaning up potential whitespace inconsistencies with beautifulsoup and spacy
...
def edit_flashcards(flashcards: pd.core.frame.DataFrame) -> pd.core.frame.DataFrame:
    ...
    while True:
        ...
        # choose index of flashcard in the current collection of our study session
        ...
        # updating all fields of the card
        ...
        if not confirm_with_user("Continue editing cards?"):
            break
...
def choose_flashcard_to_edit(flashcards: pd.core.frame.DataFrame) -> int:
    ...
    # find all rows in our dataframe where the question field matches the user's input
    ...
    # collecting all matching rows' indices
    matching_questions_indices = matching_questions.index.tolist()
    
    # printing all our matching questions, which is shown alongside the respective indices of the rows
...
def save_flashcards_to_csv(flashcards: pd.core.frame.DataFrame) -> None:
    ...
    # splitting into easy and normal difficulties, to be imported in separate decks
